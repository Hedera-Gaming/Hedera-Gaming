...
export class ServerSocketManager {
    ...
    sendUserAcceptedResponse(socketId: string, data: SpaceSim.UserData): this {
        return this.socketEmit(
            socketId,
            SpaceSim.Constants.Socket.USER_ACCEPTED,
            data
        );
    }
    ...
    sendInvalidUserDataResponse(socketId: string, message: string): this {
        return this.socketEmit(
            socketId,
            SpaceSim.Constants.Socket.INVALID_USER_DATA,
            message
        );
    }
    ...
    sendInvalidRequestEvent(socketId: string, message: string): this {
        return this.socketEmit(
            socketId,
            SpaceSim.Constants.Socket.INVALID_REQUEST,
            message
        );
    }
    ...
    sendUpdateStatsToRoom(
        ...
        return this.roomEmit(
            room,
            SpaceSim.Constants.Socket.UPDATE_STATS,
            stats
        );
    }
    ...
    sendSetShipIdResponse(socketId: string, shipId: string): this {
        return this.socketEmit(
            socketId,
            SpaceSim.Constants.Socket.SET_PLAYER_ID,
            shipId
        );
    }
    ...
    sendEngineOnEventToRoom(
        ...
        return this.roomEmit(
            room,
            SpaceSim.Constants.Socket.ENGINE_ON,
            shipId,
            enabled
        );
    }
    ...
    sendWeaponFiringEventToRoom(
        ...
        return this.roomEmit(
            room,
            SpaceSim.Constants.Socket.WEAPON_FIRING,
            shipId,
            firing
        );
    }
    ...
    sendShipDestroyedEventToRoom(room: string, shipId: string): this {
        return this.roomEmit(
            room,
            SpaceSim.Constants.Socket.SHIP_DESTROYED,
            shipId
        );
    }
    ...
    sendUpdateGameLevelToRoom(room: string, config: GameLevelConfig): this {
        return this.roomEmit(
            room,
            SpaceSim.Constants.Socket.UPDATE_MAP,
            config
        );
    }
    ...
    sendUpdatePlayersEvent(room: string, shipsState: Array<ShipState>): this {
        return this.roomEmit(
            room,
            SpaceSim.Constants.Socket.UPDATE_PLAYERS,
            shipsState
        );
    }
    ...
    sendUpdateSuppliesEvent(
        ...
        return this.roomEmit(
            room,
            SpaceSim.Constants.Socket.UPDATE_SUPPLIES,
            suppliesState
        );
    }
    ...
    sendFlickerSuppliesEvent(room: string, ...ids: Array<string>): this {
        return this.roomEmit(
            room,
            SpaceSim.Constants.Socket.FLICKER_SUPPLIES,
            ...ids
        );
    }
    ...
    sendRemoveSuppliesEvent(room: string, ...ids: Array<string>): this {
        return this.roomEmit(
            room,
            SpaceSim.Constants.Socket.REMOVE_SUPPLIES,
            ...ids
        );
    }
    ...
    private _handleDisconnectEvent(
        ...
        if (user?.room) {
            user.deleteAt =
                Date.now() + SpaceSim.Constants.Timing.DISCONNECT_TIMEOUT_MS;
            Logging.log(
                'info',
    ...
    private _tryReconnectUser(
        ...
        if (previousConnection) {
            ...
            if (updated.room) {
                const scene = SpaceSim.game.scene.getScene(
                    updated.room
        ...
        if (user) {
            ...
            for (let scene of SpaceSimServer.rooms()) {
                if (
                    ...
                    SpaceSim.Constants.Socket.MAX_USERS_PER_ROOM
                ) {
    ...
    private _handleRequestMapResponse(
        ...
        const user = SpaceSimServer.users.select(data).first;
        if (user) {
            const scene = SpaceSimServer.rooms().find(
    ...
    private _handleRequestShipResponse(
        ...
        if (user) {
            ...
            if (scene) {
                if (
                    scene.getShips().length <
                    SpaceSimServer.Constants.Rooms.MAX_BOTS
                ) {
    ...
    private _handleShipDestroyedEvent(
        ...
        if (user) {
            ...
            if (scene) {
                ...
                if (ship) {
                    scene.events.emit(
                        SpaceSim.Constants.Events.SHIP_DEATH,
                        ship.currentState
                    );
                }
            } else {
