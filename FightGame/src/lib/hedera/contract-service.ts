...
export class ContractService {
  ...
  async connect(privateKey: string) {
    ...
    const leaderboardAddress = import.meta.env
      .VITE_LEADERBOARD_CONTRACT_ADDRESS;
  ...
  async getTopPlayers(count: number = 10) {
    ...
    const [players, scores] = await this.leaderboardContract.getTopPlayers(
      count
    );
  ...
  async checkAchievement(playerAddress: string, achievementId: number) {
    ...
    return await this.verifierContract.checkAchievement(
      playerAddress,
      achievementId
    );
  }
