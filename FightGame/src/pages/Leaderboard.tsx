  ...
  useEffect(() => {
    const fetchLeaderboard = async () => {
      ...
      if (players) {
        ...
        const avgScore = ranked.reduce((sum, p) => sum + p.total_score, 0) / totalPlayers || 0;
        const totalMatches = ranked.reduce((sum, p) => sum + p.games_played, 0);
        const totalNFTs = ranked.reduce((sum, p) => sum + p.total_nfts, 0);
